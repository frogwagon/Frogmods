...
**Arrow trail color (Settings options):** four rows under the bow-charge one - an "Arrow trail color" on/off, a "Trail color" picker, a "Rainbow trail" toggle and a "Trail strength" slider (1x-5x). If the color does nothing, type `NarrowSettingsStats.trailDebug` in the console - it shows how many of your arrows it saw and patched, and why it skipped one. The game already draws a faint white trail behind every arrow; this recolors and brightens that same trail for your own arrows only (other players' arrows stay as the game draws them) by patching the trail material's shader.
