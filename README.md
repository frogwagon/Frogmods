...
**Arrow trail color (Settings options):** three rows under the bow-charge one - an "Arrow trail color" on/off, a "Trail color" picker and a "Trail strength" slider (1x-5x). The game already draws a faint white trail behind every arrow; this recolors and brightens that same trail for your own arrows only (other players' arrows stay as the game draws them) by patching the trail material's shader.
